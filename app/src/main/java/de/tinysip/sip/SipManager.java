...
public class SipManager implements SipListener {
     ...
     *
     * @param localSipProfile the local user's SIP profile to register with the provider
     * @param discoveryInfo   the STUN DiscoveryInfo for NAT traversal
     */
    private SipManager(SipProfile sipProfile, String localIPAddress, int localSipPort) {
        try {
            Log.d("SipManager(LocalSipProfile localSipProfile, String localIPAddress, int localSipPort)", "SipManager createInstance - BEGIN");
            sipMessageHandler = SipMessageHandler.createInstance(sipProfile, localIPAddress, localSipPort);
            Log.d("SipManager(LocalSipProfile localSipProfile, String localIPAddress, int localSipPort)", "SipManager createInstance - SYNC POINT");
            ...
            Log.d("SipManager(LocalSipProfile localSipProfile, String localIPAddress, int localSipPort)", "SipManager createInstance - END");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
     ...
     */
    public static SipManager createInstance(SipProfile sipProfile, String localIPAddress, int localSipPort) {
        if (sipManager == null) {
            Log.d("SipManager", "SipManager createInstance - BEGIN");
            sipManager = new SipManager(sipProfile, localIPAddress, localSipPort);
            Log.d("SipManager", "SipManager createInstance - END");
        }
    ...
    public void registerProfile() throws InvalidArgumentException, TooManyListenersException, ParseException, SipException {
        if (sipMessageHandler.getSipProfile().isLocalProfile())
     ...
     * @throws SipException
     */
    public void registerJFLProfile() throws InvalidArgumentException, TooManyListenersException, ParseException, SipException {
        setStatusChanged(SipManagerState.REGISTERING);
        sipMessageHandler.register(SipRequestState.REGISTER);
    }
     ...
     *
     * @param contact the SIP contact to call
     * @throws ParseException
     * @throws InvalidArgumentException
     * @throws SipException
        ...
        if (method.equals(Request.INVITE)) {
            try {
                if (currentSession == null) {
                    ...
                    if (currentSession.getToSipURI().equals(sipMessageHandler.getSipProfile().getSipUri())) {
        ...
        switch (status) {
            ...
            case 401: // Unauthorized
                try {
                    ...
                    sipMessageHandler.getSipProfile().setNonce(authHeader.getNonce());
                    sipMessageHandler.getSipProfile().setRealm(authHeader.getRealm());
    ...
    public SipProfile getSipProfile() {
        return sipMessageHandler.getSipProfile();
    }
     ...
     *
     * @param session the new SipSession information
     */
     ...
     *
     * @param state the new state to signal
     * @param info  additional information about the state
     */
     ...
     *
     * @param listener the listener to register for the raised events
     */
     ...
     *
     * @param listener the listener to unregister from the events
     */
